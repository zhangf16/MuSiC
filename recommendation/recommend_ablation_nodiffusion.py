...
test_new_loader = DataLoader(test_new_dataset, batch_size=512, shuffle=False)
test_none_dataset = RatingsDataset3(test_none_hdf5_path)
test_none_loader = DataLoader(test_none_dataset, batch_size=512, shuffle=False)
...
# 训练模型
def train_model(UserEmbed, ItemEmbed, train_loader1, train_loader2, test_loader, epochs, lr, weight_decay, non_overlap_weight, lamda):
    criterion = nn.MSELoss()
    optimizer = optim.Adam(list(UserEmbed.parameters()) + list(ItemEmbed.parameters()), lr=lr, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        ...
        # for user_id, user_feature, item_feature, rating, is_overlap in train_loader:
            ...
            predicted_rating = (target * item).sum(1)
        ...
        with torch.no_grad():
            for user_id, parent_asin, user_feature, item_feature, rating in test_loader:
                ...
                item = ItemEmbed(item_feature)
                predicted_rating = (user * item).sum(1)
...
def test_model_nodiffusion(UserEmbed, ItemEmbed, test_loader):
    UserEmbed.eval()
    ItemEmbed.eval()
    actuals = []
    predictions = []
    ...
    for t in [0]:
        # for dropout in [0.2]:
            for non_overlap_weight in [1.0]:
                print(f'path: {path1}, T: {T}, t: {t}, dropout: {dropout}, non_overlap_weight: {non_overlap_weight}')
                ...
                # 训练和测试
                print('training')
                best_mae, best_rmse, best_epoch, best_model_state1, best_model_state2 = \
                    train_model(UserEmbed, ItemEmbed, train_loader1, train_loader2, test_new_loader, \
                ...
                result, rmse_new, mae_new = test_model_nodiffusion(UserEmbed, ItemEmbed, test_new_loader)
                ...
                result, rmse, mae = test_model_nodiffusion(UserEmbed, ItemEmbed, test_loader)
                result, rmse_none, mae_none = test_model_nodiffusion(UserEmbed, ItemEmbed, test_none_loader)
