...
def train_model(UserEmbed, ItemEmbed, diff_model, train_loader1, train_loader2, test_loader, epochs, lr, weight_decay, non_overlap_weight, lamda):
    ...
    for epoch in range(epochs):
        ...
        # for user_id, user_feature, item_feature, rating, is_overlap in train_loader:
            ...
            click_labels = (rating >= 3).float()  # 将评分二值化
            click_prob = torch.sigmoid(predicted_rating)
            loss = nn.functional.binary_cross_entropy(click_prob, click_labels)
        ...
        rmse = mlp_model.ndcg(results_df)
        print(f"Epoch {epoch}, ndcg: {rmse:.5f}", end='\t')
        ...
        # for attention in [[], [0],[0,1]]:
            for dropout in [0.2]:
                # for non_overlap_weight in [1.0]:
                    for lamda in [0.4]:
                        print(f'lr: {lr}, T: {T}, dropout: {dropout}, non_overlap_weight: {non_overlap_weight}, lamda: {lamda}, hidden_dims: {hidden_dims}')
