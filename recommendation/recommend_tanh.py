...
# path1 = "/root/autodl-tmp/multimodal_diffusion/data/book2movie/5/"
# path1 = "/root/autodl-tmp/multimodal_diffusion/data/book2music/5/"
...
def train_model(UserEmbed, ItemEmbed, diff_model, train_loader1, train_loader2, test_loader, epochs, lr, weight_decay, non_overlap_weight, lamda):
    ...
    for epoch in range(epochs):
        ...
        # for user_id, user_feature, item_feature, rating, is_overlap in train_loader:
            ...
            predicted_rating = (predicted_feature * item).sum(1)
            loss = criterion(predicted_rating, rating.float())
            total_loss = (lamda * diffusion_loss + (1 - lamda) * loss) * (1 - non_overlap_weight)
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
...
for T in [5]:
    # t_values = sorted(set([0,T//4,T//2,T]))
    # for t in t_values:
    # for hidden_dims in [[32*2, 32*4]]:
    #     for attention in [[], [0]]:
        # for dropout in [0.1,0.2]:
                for non_overlap_weight in [0.2,0.4,0.6,0.8]:
                    for lamda in [0.2,0.4]:
                        print(f'lr: {lr}, T: {T}, t: {t}, non_overlap_weight: {non_overlap_weight}, lamda: {lamda}, hidden_dims: {hidden_dims}, attention: {attention}')
                        ...
                        result, rmse_new, mae_new = mlp_model.test_model(UserEmbed, ItemEmbed, diff_model, test_new_loader)
                        ...
                        result, rmse, mae = mlp_model.test_model(UserEmbed, ItemEmbed, diff_model, test_loader)
                        result, rmse_none, mae_none = mlp_model.test_model(UserEmbed, ItemEmbed, diff_model, test_none_loader)
