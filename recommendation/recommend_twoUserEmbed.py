...
# 训练模型
def train_model(UserEmbed, UserEmbed2, ItemEmbed, diff_model, train_loader1, train_loader2, test_loader, epochs, lr, weight_decay, non_overlap_weight, lamda):
    criterion = nn.MSELoss()
    optimizer = optim.Adam(list(UserEmbed.parameters()) + list(UserEmbed2.parameters()) + list(ItemEmbed.parameters()) + list(diff_model.parameters()), lr=lr, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        ...
        # for user_id, user_feature, item_feature, rating, is_overlap in train_loader:
            ...
            target = UserEmbed2(user_feature_t)
            item = ItemEmbed(item_feature)
            ...
            for ratio in ["2/","5/","8/"]:
                # for non_overlap_weight in [1.0]:
                    # for lamda in [0.4]:
                        ...
                        UserEmbed2 = UserEmbedding(input_size=4096, hidden_size=128, output_size=32, dropout_rate=dropout).to(device)
                        ...
                        best_mae, best_rmse, best_epoch, best_model_state1, best_model_state2, best_model_state3 = \
                            train_model(UserEmbed, UserEmbed2, ItemEmbed, diff_model, train_loader1, train_loader2, test_new_loader, \
