...
class DiffCDR(nn.Module):
    def __init__(self,num_steps=200, in_features=32,c_scale=0.1,diff_sample_steps=30,diff_task_lambda=0.1,diff_mask_rate=0.1 ):
    ...
    def forward(self, x,t, cond_emb,cond_mask ):
        ...
        for idx in range( self.num_layers ):
            ...
            # cond_emb = self.cond_emb_linear[idx](cond_emb)

            x = x + t_embedding + cond_emb * cond_mask.unsqueeze(-1)
            #x= torch.cat([t_embedding,cond_embedding * cond_mask.unsqueeze(-1),x],axis=1)

            # x = torch.tanh(self.linears[0](x))
